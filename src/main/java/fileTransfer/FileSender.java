...
public class FileSender implements Runnable, IFileTransfer {
  ...
  public FileSender(Buddy b, String fileName) {
    ...
    ThreadManager
        .registerWork(ThreadManager.NORMAL, this, "File Send Thread [" + b + ", " + fileName + "]",
            "fs[" + b + ", " + fileName + "]");
  }
  ...
  public void run() {
    ...
    try {
      ...
      if (this.fileSize == 0) {
        Gui.getChatWindow(buddy, true, true)
            .append("Time Stamp", "(" + GuiChatWindow.getTime() + ") ");
        Gui.getChatWindow(buddy, true, true).append("Them",
            " --> " + "Can not start File-Transfer, the file has the size 0" + "\n");
      } else {
      ...
      for (i = 0; i < blocks; i++) {
        ...
        if (start >= first) {
          ...
          synchronized (buddy.getTsoLock()) {
            OutputStream os = buddy.getTheirSock().getOutputStream();
            String msg =
                "filedata " + id + " " + start + " " + hash + " "; // + new String(data.array());
            Logger.log(Logger.NOTICE, this.getClass(), "Sending " + msg + " to " + buddy + ".");
