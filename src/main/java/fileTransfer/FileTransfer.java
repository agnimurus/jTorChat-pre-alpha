...
public class FileTransfer {
  ...
  public static void in_filename(Buddy buddy, String command, InputStream is) {
    ...
    String fileName = spl[3].replaceAll((char) 0 + "", "").replaceAll("/", "")
        .replaceAll("\\\\", "");
    ...
    Logger.log(Logger.NOTICE, "FileTransfer",
        "filename recieved from " + buddy + " | " + id + ", " + fileSize + ", " + blockSize + ", "
            + fileName + " | " + base + ", " + ext);
  ...
  public static void in_file_stop_sending(Buddy buddy, String s) {
    ...
    String id = s.split(COMMAND_SEPARATOR,
        2)[1]; // NOTE - in pytorchat this is done by self.id = self.blob doesnt make sense
    FileSender sender = senders.get(buddy.getAddress() + COMMAND_SEPARATOR + id);
    if (sender != null) {
      sender.close();
    }
