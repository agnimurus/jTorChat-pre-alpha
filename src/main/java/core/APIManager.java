...
public class APIManager {
  ...
  public static void addEventListener(APIListener apiListener) {
    listeners.add(apiListener);
  }

  public static void fireStatusChange(Buddy buddy, byte newStatus, byte oldStatus) {
    try {
      Logger.log(Logger.DEBUG, "API",
          buddy.getAddress() + " status change from " + Buddy.getStatusName(oldStatus) + " to "
              + Buddy.getStatusName(newStatus));
  ...
  public static void fireProfileNameChange(Buddy buddy, String newName, String oldName) {
    try {
      Logger.log(Logger.DEBUG, "API",
          buddy.getAddress() + " name changed from " + oldName + " to " + newName);
      for (APIListener apil : listeners) {
  ...
  public static void fireProfileTextChange(Buddy buddy, String newText, String oldText) {
    try {
      Logger.log(Logger.DEBUG, "API",
          buddy.getAddress() + " profileText changed from " + oldText + " to " + newText);
      for (APIListener apil : listeners) {
  ...
  public static void fireMessage(Buddy buddy, String message) {
    try {
      Logger.log(Logger.DEBUG, "API", buddy.getAddress() + " sent " + message);
      for (APIListener apil : listeners) {
        apil.onMessage(buddy, message);
      }
    } catch (Exception e) {
      e.printStackTrace();
    }
