    ...
    ThreadManager.registerWork(ThreadManager.DAEMON, () -> {
      try {
        try {
          ...
          Logger.log(Logger.FATAL, "TCServ",
              "Failed to start local server: " + e.getLocalizedMessage());
          return;
        }
        ...
        while (serverSock.isBound() && !serverSock.isClosed() && running) {
          ...
          ThreadManager.registerWork(ThreadManager.NORMAL, () -> {
            try {
              ...
              if (BuddyList.buds.containsKey(line.split(
                  " ")[1])) { // TODO add check to see if its different cookie from last time
                Logger.log(Logger.INFO, "TCServ",
                    "Got ping from " + line.split(" ")[1] + " with cookie " + line.split(" ")[2]);
                Buddy buddy = BuddyList.buds.get(line.split(" ")[1]);
                Logger.log(Logger.INFO, "TCServ",
                    "Match " + line.split(" ")[1] + " to " + buddy.getAddress());
                ...
                if (line.split(" ")[1].length() == 16)  // first defend for flooding ping
                {
                  if (!line.split(" ")[1].equals(Config.getUs())) {
                    ...
                    Logger.log(Logger.INFO, "TCServ",
                        "Got ping from unknown address " + line.split(" ")[1] + " with cookie "
                            + line.split(" ")[2]);
                    buddy.setTheirCookie(line.split(" ")[2]);
                    if (buddy.getOurSock() == null) {
                      buddy.connect();
                      ...
                      buddy.sendPong(
                          line.split(" ")[2]); // TODO FIXME URGENT check if not connected!
                    }
                    buddy.attach(sock, sc);
                  }
                }
