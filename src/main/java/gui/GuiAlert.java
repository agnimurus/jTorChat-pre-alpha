...
public class GuiAlert extends Thread {
  ...
  private void doAWTUtilities() {
    try {
      ...
      if ((Boolean) awtuc.getMethod("isTranslucencySupported", awtutc)
          .invoke(null, awtutc.getDeclaredField("PERPIXEL_TRANSLUCENT").get(null))
          && (Boolean) awtuc.getMethod("isTranslucencyCapable", GraphicsConfiguration.class)
          .invoke(null, f.getGraphicsConfiguration())) {
  ...
  public void run() {
    ...
    f.setLocation(mwb.x + mwb.width - dispWidth - 1,
        mwb.y + mwb.height - dispHeight - 1 - dispHeight * (numAlerts.get() - 1) - (numAlerts.get()
            - 1));
    f.setVisible(true);
    while (running && pr-- > 1) {
      try {
