...
public class GuiPingAttack extends JFrame {

  // public static void main(String[] a) {
  // new GUIContactAdd().setVisible(true);
  // }

  public GuiPingAttack() {
    setResizable(false);
  ...
  private void ok(ActionEvent e) {
    ...
    if (addr.length() != 16) {
      JOptionPane.showMessageDialog(null, Language.langtext[55], Language.langtext[56],
          JOptionPane.ERROR_MESSAGE);
      return;
    }
    int count;
    try {
      count = Integer.parseInt(textField2.getText());
    } catch (Exception e1) {
      JOptionPane
          .showMessageDialog(null, "Problem", "Count need a int.", JOptionPane.ERROR_MESSAGE);
      return;
    }

    Ping_Attack.attack(addr, count);
    dispose();

  }

  private void language() {
    label1.setText("Tor-ID");
