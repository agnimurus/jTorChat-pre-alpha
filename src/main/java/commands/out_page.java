...
public class out_page {
  ...
  public static void command(Buddy buddy, String command, GuiChatWindow window, boolean withDelay) {
    ...
    if (Config.getPageactive() == 1) {
      ...
      command = "/page " + command.substring(6)
          .replaceAll("[^a-zA-Z]", ""); // Replace all special letters

      String msg = util_page.read(command.substring(6));
      if (msg == "") {
        ChatWindow.update_window(MessageType.RECEIVE_PAGE, window,
            "He try to get '" + command.substring(6) + "' but it does not exist.", "",
            "/pa You try to get '" + command.substring(6) + "' but it does not exist.", withDelay);
      } else {
        ChatWindow
            .update_window(MessageType.RECEIVE_PAGE, window, "He get " + command.substring(6) + msg,
                "", "/pa Get " + command.substring(6) + msg, withDelay);
      }
    } else {
      ChatWindow.update_window(MessageType.PRIVATE, window,
          "He try to get something but the page system is not activated.", "",
          "/pa You try to get something but the page system is not activated..", withDelay);
    }
