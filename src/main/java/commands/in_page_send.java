...
public class in_page_send {
  ...
  public static void command(Buddy buddy, String command, GuiChatWindow window) {
    ...
    command = "/page_send " + command.substring(11)
    ...
    if (msg == "")//See comment T1 in ChatWindow
    {
      ChatWindow.update_window(MessageType.PRIVATE, window,
          "The page '" + command.substring(11) + "' does not exist.", "", "", false);
    } else {
      ChatWindow
          .update_window(MessageType.SEND_PAGE, window, "Get '" + command.substring(11) + "'" + msg,
              "", "/pa Get " + command.substring(11) + msg, !buddy.isFullyConnected());
    }
