...
public class RequestHTTP {
  ...
  public static ArrayList<String> load(String remote_URL) {
    ...
    Logger.log(Logger.INFO, CLASS_NAME,
        "Connect to proxy with port: " + Config.getSocksPort() + " and host: 127.0.0.1");
    Socket ourSock = new Socket(new Proxy(Proxy.Type.SOCKS, new InetSocketAddress("127.0.0.1",
        Config.getSocksPort())));
    Logger.log(Logger.INFO, CLASS_NAME,
        "Starting secure remote proxy tunnel to " + host + " with port " + port);
    try {
    ...
    if (remote_URL.startsWith("https://")) {
      ...
      SSLSocket socket;
      try {
        socket = (SSLSocket) factory
            .createSocket(ourSock, "127.0.0.1", Config.getSocksPort(), true);
